...
# Tidygraph is amazing
migration_graph <- migration_raw %>%
...
migration_graph %>% 
  ggraph("hive", axis = mun, normalize = FALSE) +
  geom_node_point(
    aes(size = value), fill = NA, color = "#3a3e4c", pch = 21, alpha = .7
  ) +
  geom_edge_hive(
    aes(color = gender, edge_width = value, alpha = value),
    strength = 15
  ) +
  geom_node_text(
    aes(label = lbl), size = 3, nudge_x = .03, nudge_y = -.01, 
  ...
  labs(
    title = "Migration", subtitle = "Greenland 2020", 
